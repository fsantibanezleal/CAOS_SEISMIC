...
# CAOS_SEISMIC: forecast records are results, not content. The per-run manifests, the published
# results, the raw data and the app's generated data fallback keep the text their producers wrote.
RECORD_DIRS = {"manifests", "results", "data"}
RECORD_PREFIXES = ("app/public/data/",)
...
def main() -> int:
    ...
    for rel in tracked_files():
        if Path(rel).parts[0] in RECORD_DIRS or rel.startswith(RECORD_PREFIXES):
            continue
